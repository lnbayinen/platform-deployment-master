...
# InSpec Control file for rsa-audit-server NetWitness 11 component


# Component firewall testing
control 'rsa-audit-server-firewall' do
  impact 1.0
  title 'rsa-audit-server-firewall test'
  ...
  if node_json['rsa-audit-server']['firewall_rules']
    node_json['rsa-audit-server']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'rsa-audit-server-ports' do
#   impact 1.0
#   title 'rsa-audit-server-prts test'
...
control 'rsa-audit-server-accounts' do
  impact 1.0
  title 'rsa-audit-server-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-audit-server']['accounts'].each do |account|
...
control 'rsa-audit-server-packages' do
  impact 1.0
  title 'rsa-audit-server-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-audit-server']['packages'].each do |package|
...
control 'rsa-audit-server-services' do
  impact 1.0
  title 'rsa-audit-server-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['rsa-audit-server']['service_names']
    node_json['rsa-audit-server']['service_names'].each do |service_name|
...
control 'rsa-audit-server-filesystem' do
  impact 1.0
  title 'rsa-audit-server-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['rsa-audit-server']['filesystem']
    if node_json['rsa-audit-server']['filesystem']['directories']
      node_json['rsa-audit-server']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['rsa-audit-server']['filesystem']['symlinks']
      node_json['rsa-audit-server']['filesystem']['symlinks'].each do |symlink|
