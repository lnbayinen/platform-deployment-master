...
# InSpec Control file for rsa-audit NetWitness 11 component


# Component firewall testing
control 'rsa-audit-firewall' do
  impact 1.0
  title 'rsa-audit-firewall test'
  ...
  if node_json['rsa-audit']['firewall_rules']
    node_json['rsa-audit']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'rsa-audit-ports' do
#   impact 1.0
#   title 'rsa-audit-prts test'
...
control 'rsa-audit-accounts' do
  impact 1.0
  title 'rsa-audit-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-audit']['accounts'].each do |account|
...
control 'rsa-audit-packages' do
  impact 1.0
  title 'rsa-audit-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-audit']['packages'].each do |package|
...
control 'rsa-audit-services' do
  impact 1.0
  title 'rsa-audit-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['rsa-audit']['service_names']
    node_json['rsa-audit']['service_names'].each do |service_name|
...
control 'rsa-audit-filesystem' do
  impact 1.0
  title 'rsa-audit-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['rsa-audit']['filesystem']
    if node_json['rsa-audit']['filesystem']['directories']
      node_json['rsa-audit']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['rsa-audit']['filesystem']['symlinks']
      node_json['rsa-audit']['filesystem']['symlinks'].each do |symlink|
