...
# InSpec Control file for rsa-sms-runtime NetWitness 11 component


# Component firewall testing
control 'rsa-sms-runtime-firewall' do
  impact 1.0
  title 'rsa-sms-runtime-firewall test'
  ...
  if node_json['rsa-sms-runtime']['firewall_rules']
    node_json['rsa-sms-runtime']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'rsa-sms-runtime-ports' do
#   impact 1.0
#   title 'rsa-sms-runtime-prts test'
...
control 'rsa-sms-runtime-accounts' do
  impact 1.0
  title 'rsa-sms-runtime-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-sms-runtime']['accounts'].each do |account|
...
control 'rsa-sms-runtime-packages' do
  impact 1.0
  title 'rsa-sms-runtime-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-sms-runtime']['packages'].each do |package|
...
control 'rsa-sms-runtime-services' do
  impact 1.0
  title 'rsa-sms-runtime-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['rsa-sms-runtime']['service_names']
    node_json['rsa-sms-runtime']['service_names'].each do |service_name|
...
control 'rsa-sms-runtime-filesystem' do
  impact 1.0
  title 'rsa-sms-runtime-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['rsa-sms-runtime']['filesystem']
    if node_json['rsa-sms-runtime']['filesystem']['directories']
      node_json['rsa-sms-runtime']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['rsa-sms-runtime']['filesystem']['symlinks']
      node_json['rsa-sms-runtime']['filesystem']['symlinks'].each do |symlink|
