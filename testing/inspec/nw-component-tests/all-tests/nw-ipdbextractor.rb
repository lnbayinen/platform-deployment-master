...
# InSpec Control file for nw-ipdbextractor NetWitness 11 component


# Component firewall testing
control 'nw-ipdbextractor-firewall' do
  impact 1.0
  title 'nw-ipdbextractor-firewall test'
  ...
  if node_json['nw-ipdbextractor']['firewall_rules']
    node_json['nw-ipdbextractor']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'nw-ipdbextractor-ports' do
#   impact 1.0
#   title 'nw-ipdbextractor-prts test'
...
control 'nw-ipdbextractor-accounts' do
  impact 1.0
  title 'nw-ipdbextractor-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-ipdbextractor']['accounts'].each do |account|
...
control 'nw-ipdbextractor-packages' do
  impact 1.0
  title 'nw-ipdbextractor-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-ipdbextractor']['packages'].each do |package|
...
control 'nw-ipdbextractor-services' do
  impact 1.0
  title 'nw-ipdbextractor-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['nw-ipdbextractor']['service_names']
    node_json['nw-ipdbextractor']['service_names'].each do |service_name|
...
control 'nw-ipdbextractor-collectd-ipdbextractor-conf' do
    impact 1.0
    title 'nw-ipdbextractor-collectd-ipdbextractor-conf'
    desc 'Ensure /etc/collectd.d/ipdbextractor.conf file is present'
    describe file('/etc/collectd.d/ipdbextractor.conf') do
        it {should be_file}
...
control 'nw-ipdbextractor-filesystem' do
  impact 1.0
  title 'nw-ipdbextractor-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['nw-ipdbextractor']['filesystem']
    if node_json['nw-ipdbextractor']['filesystem']['directories']
      node_json['nw-ipdbextractor']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['nw-ipdbextractor']['filesystem']['symlinks']
      node_json['nw-ipdbextractor']['filesystem']['symlinks'].each do |symlink|
