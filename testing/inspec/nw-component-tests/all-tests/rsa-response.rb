...
# InSpec Control file for rsa-response NetWitness 11 component


# Component firewall testing
control 'rsa-response-firewall' do
  impact 1.0
  title 'rsa-response-firewall test'
  ...
  if node_json['rsa-response']['firewall_rules']
    node_json['rsa-response']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'rsa-response-ports' do
#   impact 1.0
#   title 'rsa-response-prts test'
...
control 'rsa-response-accounts' do
  impact 1.0
  title 'rsa-response-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-response']['accounts'].each do |account|
...
control 'rsa-response-packages' do
  impact 1.0
  title 'rsa-response-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-response']['packages'].each do |package|
...
control 'rsa-response-services' do
  impact 1.0
  title 'rsa-response-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['rsa-response']['service_names']
    node_json['rsa-response']['service_names'].each do |service_name|
...
control 'rsa-response-filesystem' do
  impact 1.0
  title 'rsa-response-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['rsa-response']['filesystem']
    if node_json['rsa-response']['filesystem']['directories']
      node_json['rsa-response']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['rsa-response']['filesystem']['symlinks']
      node_json['rsa-response']['filesystem']['symlinks'].each do |symlink|
