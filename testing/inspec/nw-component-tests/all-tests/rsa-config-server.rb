...
# InSpec Control file for rsa-config-server NetWitness 11 component


# Component firewall testing
control 'rsa-config-server-firewall' do
  impact 1.0
  title 'rsa-config-server-firewall test'
  ...
  if node_json['rsa-config-server']['firewall_rules']
    node_json['rsa-config-server']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'rsa-config-server-ports' do
#   impact 1.0
#   title 'rsa-config-server-prts test'
...
control 'rsa-config-server-accounts' do
  impact 1.0
  title 'rsa-config-server-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-config-server']['accounts'].each do |account|
...
control 'rsa-config-server-packages' do
  impact 1.0
  title 'rsa-config-server-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-config-server']['packages'].each do |package|
...
control 'rsa-config-server-services' do
  impact 1.0
  title 'rsa-config-server-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['rsa-config-server']['service_names']
    node_json['rsa-config-server']['service_names'].each do |service_name|
...
control 'rsa-config-server-filesystem' do
  impact 1.0
  title 'rsa-config-server-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['rsa-config-server']['filesystem']
    if node_json['rsa-config-server']['filesystem']['directories']
      node_json['rsa-config-server']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['rsa-config-server']['filesystem']['symlinks']
      node_json['rsa-config-server']['filesystem']['symlinks'].each do |symlink|
