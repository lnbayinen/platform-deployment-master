...
# InSpec Control file for salt-api NetWitness 11 component


# Component firewall testing
control 'salt-api-firewall' do
  impact 1.0
  title 'salt-api-firewall test'
  ...
  if node_json['salt-api']['firewall_rules']
    node_json['salt-api']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'salt-api-ports' do
#   impact 1.0
#   title 'salt-api-prts test'
...
control 'salt-api-accounts' do
  impact 1.0
  title 'salt-api-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['salt-api']['accounts'].each do |account|
...
control 'salt-api-packages' do
  impact 1.0
  title 'salt-api-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['salt-api']['packages'].each do |package|
...
control 'salt-api-services' do
  impact 1.0
  title 'salt-api-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['salt-api']['service_names']
    node_json['salt-api']['service_names'].each do |service_name|
...
control 'salt-api-filesystem' do
  impact 1.0
  title 'salt-api-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['salt-api']['filesystem']
    if node_json['salt-api']['filesystem']['directories']
      node_json['salt-api']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['salt-api']['filesystem']['symlinks']
      node_json['salt-api']['filesystem']['symlinks'].each do |symlink|
