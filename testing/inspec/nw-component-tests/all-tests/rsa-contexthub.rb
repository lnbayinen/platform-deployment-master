...
# InSpec Control file for rsa-contexthub NetWitness 11 component


# Component firewall testing
control 'rsa-contexthub-firewall' do
  impact 1.0
  title 'rsa-contexthub-firewall test'
  ...
  if node_json['rsa-contexthub']['firewall_rules']
    node_json['rsa-contexthub']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'rsa-contexthub-ports' do
#   impact 1.0
#   title 'rsa-contexthub-prts test'
...
control 'rsa-contexthub-accounts' do
  impact 1.0
  title 'rsa-contexthub-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-contexthub']['accounts'].each do |account|
...
control 'rsa-contexthub-packages' do
  impact 1.0
  title 'rsa-contexthub-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-contexthub']['packages'].each do |package|
...
control 'rsa-contexthub-services' do
  impact 1.0
  title 'rsa-contexthub-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['rsa-contexthub']['service_names']
    node_json['rsa-contexthub']['service_names'].each do |service_name|
...
control 'rsa-contexthub-filesystem' do
  impact 1.0
  title 'rsa-contexthub-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['rsa-contexthub']['filesystem']
    if node_json['rsa-contexthub']['filesystem']['directories']
      node_json['rsa-contexthub']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['rsa-contexthub']['filesystem']['symlinks']
      node_json['rsa-contexthub']['filesystem']['symlinks'].each do |symlink|
