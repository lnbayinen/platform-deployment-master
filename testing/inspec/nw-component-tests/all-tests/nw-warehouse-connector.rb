...
# InSpec Control file for nw-warehouse-connector NetWitness 11 component


# Component firewall testing
control 'nw-warehouse-connector-firewall' do
  impact 1.0
  title 'nw-warehouse-connector-firewall test'
  ...
  if node_json['nw-warehouse-connector']['firewall_rules']
    node_json['nw-warehouse-connector']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'nw-warehouse-connector-ports' do
#   impact 1.0
#   title 'nw-warehouse-connector-prts test'
...
control 'nw-warehouse-connector-accounts' do
  impact 1.0
  title 'nw-warehouse-connector-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-warehouse-connector']['accounts'].each do |account|
...
control 'nw-warehouse-connector-packages' do
  impact 1.0
  title 'nw-warehouse-connector-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-warehouse-connector']['packages'].each do |package|
...
control 'nw-warehouse-connector-services' do
  impact 1.0
  title 'nw-warehouse-connector-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['nw-warehouse-connector']['service_names']
    node_json['nw-warehouse-connector']['service_names'].each do |service_name|
...
control 'nw-warehouse-connector-filesystem' do
  impact 1.0
  title 'nw-warehouse-connector-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['nw-warehouse-connector']['filesystem']
    if node_json['nw-warehouse-connector']['filesystem']['directories']
      node_json['nw-warehouse-connector']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['nw-warehouse-connector']['filesystem']['symlinks']
      node_json['nw-warehouse-connector']['filesystem']['symlinks'].each do |symlink|
