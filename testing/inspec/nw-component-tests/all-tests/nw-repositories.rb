...
# InSpec Control file for nw-repositories NetWitness 11 component


# Component firewall testing
control 'nw-repositories-firewall' do
  impact 1.0
  title 'nw-repositories-firewall test'
  ...
  if node_json['nw-repositories']['firewall_rules']
    node_json['nw-repositories']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'nw-repositories-ports' do
#   impact 1.0
#   title 'nw-repositories-prts test'
...
control 'nw-repositories-accounts' do
  impact 1.0
  title 'nw-repositories-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-repositories']['accounts'].each do |account|
...
control 'nw-repositories-packages' do
  impact 1.0
  title 'nw-repositories-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-repositories']['packages'].each do |package|
...
control 'nw-repositories-services' do
  impact 1.0
  title 'nw-repositories-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['nw-repositories']['service_names']
    node_json['nw-repositories']['service_names'].each do |service_name|
...
control 'nw-repositories-filesystem' do
  impact 1.0
  title 'nw-repositories-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['nw-repositories']['filesystem']
    if node_json['nw-repositories']['filesystem']['directories']
      node_json['nw-repositories']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['nw-repositories']['filesystem']['symlinks']
      node_json['nw-repositories']['filesystem']['symlinks'].each do |symlink|
