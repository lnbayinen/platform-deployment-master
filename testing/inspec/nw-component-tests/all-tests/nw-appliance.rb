...
# InSpec Control file for nw-appliance NetWitness 11 component


# Component firewall testing
control 'nw-appliance-firewall' do
  impact 1.0
  title 'nw-appliance-firewall test'
  ...
  if node_json['nw-appliance']['firewall_rules']
    node_json['nw-appliance']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'nw-appliance-ports' do
#   impact 1.0
#   title 'nw-appliance-prts test'
...
control 'nw-appliance-accounts' do
  impact 1.0
  title 'nw-appliance-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-appliance']['accounts'].each do |account|
...
control 'nw-appliance-packages' do
  impact 1.0
  title 'nw-appliance-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-appliance']['packages'].each do |package|
...
control 'nw-appliance-services' do
  impact 1.0
  title 'nw-appliance-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['nw-appliance']['service_names']
    node_json['nw-appliance']['service_names'].each do |service_name|
...
control 'nw-appliance-filesystem' do
  impact 1.0
  title 'nw-appliance-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['nw-appliance']['filesystem']
    if node_json['nw-appliance']['filesystem']['directories']
      node_json['nw-appliance']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['nw-appliance']['filesystem']['symlinks']
      node_json['nw-appliance']['filesystem']['symlinks'].each do |symlink|
