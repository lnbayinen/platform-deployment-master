...
# InSpec Control file for nw-ntp NetWitness 11 component


# Component firewall testing
control 'nw-ntp-firewall' do
  impact 1.0
  title 'nw-ntp-firewall test'
  ...
  if node_json['nw-ntp']['firewall_rules']
    node_json['nw-ntp']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'nw-ntp-ports' do
#   impact 1.0
#   title 'nw-ntp-prts test'
...
control 'nw-ntp-accounts' do
  impact 1.0
  title 'nw-ntp-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-ntp']['accounts'].each do |account|
...
control 'nw-ntp-packages' do
  impact 1.0
  title 'nw-ntp-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['nw-ntp']['packages'].each do |package|
...
control 'nw-ntp-services' do
  impact 1.0
  title 'nw-ntp-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['nw-ntp']['service_names']
    node_json['nw-ntp']['service_names'].each do |service_name|
...
control 'nw-ntp-filesystem' do
  impact 1.0
  title 'nw-ntp-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['nw-ntp']['filesystem']
    if node_json['nw-ntp']['filesystem']['directories']
      node_json['nw-ntp']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['nw-ntp']['filesystem']['symlinks']
      node_json['nw-ntp']['filesystem']['symlinks'].each do |symlink|
