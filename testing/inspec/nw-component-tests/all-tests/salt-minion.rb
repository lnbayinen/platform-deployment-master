...
# InSpec Control file for salt-minion NetWitness 11 component


# Component firewall testing
control 'salt-minion-firewall' do
  impact 1.0
  title 'salt-minion-firewall test'
  ...
  if node_json['salt-minion']['firewall_rules']
    node_json['salt-minion']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'salt-minion-ports' do
#   impact 1.0
#   title 'salt-minion-prts test'
...
control 'salt-minion-accounts' do
  impact 1.0
  title 'salt-minion-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['salt-minion']['accounts'].each do |account|
...
control 'salt-minion-packages' do
  impact 1.0
  title 'salt-minion-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['salt-minion']['packages'].each do |package|
...
control 'salt-minion-services' do
  impact 1.0
  title 'salt-minion-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['salt-minion']['service_names']
    node_json['salt-minion']['service_names'].each do |service_name|
...
control 'salt-minion-filesystem' do
  impact 1.0
  title 'salt-minion-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['salt-minion']['filesystem']
    if node_json['salt-minion']['filesystem']['directories']
      node_json['salt-minion']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['salt-minion']['filesystem']['symlinks']
      node_json['salt-minion']['filesystem']['symlinks'].each do |symlink|
