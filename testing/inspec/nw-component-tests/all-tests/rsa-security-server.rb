...
# InSpec Control file for rsa-security-server NetWitness 11 component


# Component firewall testing
control 'rsa-security-server-firewall' do
  impact 1.0
  title 'rsa-security-server-firewall test'
  ...
  if node_json['rsa-security-server']['firewall_rules']
    node_json['rsa-security-server']['firewall_rules'].each do |rule|
      case rule['protocol']
      when 'tcp'
...
# control 'rsa-security-server-ports' do
#   impact 1.0
#   title 'rsa-security-server-prts test'
...
control 'rsa-security-server-accounts' do
  impact 1.0
  title 'rsa-security-server-accounts test'
  desc 'Ensure service or system have been applied'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-security-server']['accounts'].each do |account|
...
control 'rsa-security-server-packages' do
  impact 1.0
  title 'rsa-security-server-packages test'
  desc 'Ensure all have been installed and they are in the correct version'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  node_json['rsa-security-server']['packages'].each do |package|
...
control 'rsa-security-server-services' do
  impact 1.0
  title 'rsa-security-server-services test'
  desc 'Ensure all component services are running'

  # Read data from node.json
  node_json = json('/opt/rsa/platform/nw-chef/node.json')

  if node_json['rsa-security-server']['service_names']
    node_json['rsa-security-server']['service_names'].each do |service_name|
...
control 'rsa-security-server-filesystem' do
  impact 1.0
  title 'rsa-security-server-filesystem'
  desc 'Ensure various directories and symlinks are present'
  node_json = json('/opt/rsa/platform/nw-chef/node.json')
  if node_json['rsa-security-server']['filesystem']
    if node_json['rsa-security-server']['filesystem']['directories']
      node_json['rsa-security-server']['filesystem']['directories'].each do |directory|
        describe file(directory['path']) do
          it {should be_directory}
        end
      end
    end
    if node_json['rsa-security-server']['filesystem']['symlinks']
      node_json['rsa-security-server']['filesystem']['symlinks'].each do |symlink|
