...
template '/etc/collectd.d/workbench.conf' do
  source 'workbench.conf.erb'
  owner 'root'
  group 'root'
  mode 0o444
  variables(
...
remote_file '/usr/lib/collectd/python/comp_modules/cs_workbench.py' do
  source 'file:///usr/lib/collectd/python/cs_workbench.py'
  owner 'root'
  group 'root'
  mode 0o444
  notifies :restart, 'service[collectd]', :delayed
