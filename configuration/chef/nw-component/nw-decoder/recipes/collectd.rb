...
template '/etc/collectd.d/decoder.conf' do
  source 'decoder.conf.erb'
  owner 'root'
  group 'root'
  mode 0o444
  variables(
...
remote_file '/usr/lib/collectd/python/comp_modules/cs_decoder.py' do
  source 'file:///usr/lib/collectd/python/cs_decoder.py'
  owner 'root'
  group 'root'
  mode 0o444
  notifies :restart, 'service[collectd]', :delayed
