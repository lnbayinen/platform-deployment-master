...
describe 'nw-log-decoder::collectd' do
  let(:chef_run) do
  ...
  context 'When deploying the python script' do
    file = '/usr/lib/collectd/python/comp_modules/cs_logdecoder.py'
    properties = {
      owner: 'root',
      group: 'root',
      mode: 0o444
  ...
  context 'When deploying the collectd configuration fragment' do
    file = '/etc/collectd.d/logdecoder.conf'
    properties = {
      owner: 'root',
      group: 'root',
      mode: 0o444
  ...
  context 'When deploying the collectd ESM configuration fragment' do
    file = '/etc/collectd.d/logdecoder-esm.conf'
    properties = {
      owner: 'root',
      group: 'root',
      mode: 0o444
