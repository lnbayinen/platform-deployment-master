# From https://github.com/rabbitmq/chef-cookbook/blob/master/providers/user.rb
...
def user_has_permissions?(name, vhost, perm_list = nil)
  ...
  cmd = Mixlib::ShellOut.new(cmd, env: { 'HOME' => '/var/lib/rabbitmq' })
  cmd.run_command
  Chef::Log.debug "user_has_permissions?: #{cmd}"
  Chef::Log.debug "user_has_permissions?: #{cmd.stdout}"
  Chef::Log.debug "user_has_permissions?: #{cmd.exitstatus}"
  if perm_list.nil? && cmd.stdout.empty? # looking for empty and found nothing
    Chef::Log.debug 'user_has_permissions?: no permissions found'
    return false
  end
  if perm_list == cmd.stdout.split.drop(1) # existing match search
    Chef::Log.debug 'user_has_permissions?: matching permissions already found'
    return true
  end
  Chef::Log.debug 'user_has_permissions?: permissions found but do not match'
  false
end
...
action :add do
  unless user_exists?(new_resource.user)
    Chef::Application.fatal!(
      'rabbitmq_user with action :add requires a non-empty password.'
    ) if new_resource.password.nil? || new_resource.password.empty?

    # To escape single quotes in a shell, you have to close the surrounding
    # single quotes, add in an escaped single quote, and then re-open the
    # original single quotes. Since this string is interpolated once by ruby,
    # and then a second time by the shell, we need to escape the escape
    # character ('\') twice.  This is why the following is such a mess of
    # leaning toothpicks:
...
action :set_permissions do
  Chef::Application.fatal!(
    ":set_permissions fails with non-existant '#{new_resource.user}' user."
  ) unless user_exists?(new_resource.user)

  perm_list = new_resource.permissions.split
  vhosts = Array(new_resource.vhost)
  vhosts.each do |vhost|
    next if user_has_permissions?(new_resource.user, vhost, perm_list)
    vhostopt = "-p #{vhost}" unless vhost.nil?
    cmd =  "rabbitmqctl set_permissions #{vhostopt} "
    cmd << "#{new_resource.user} \"#{perm_list.join('" "')}\""
    execute cmd do
      environment('HOME' => '/var/lib/rabbitmq')
...
action :clear_permissions do
  Chef::Application.fatal!(
    ":clear_permissions fails with non-existant '#{new_resource.user}' user."
  ) unless user_exists?(new_resource.user)
...
action :set_tags do
  Chef::Application.fatal!(
    ":set_tags fails with non-existant '#{new_resource.user}' user."
  ) unless user_exists?(new_resource.user)
...
action :clear_tags do
  Chef::Application.fatal!(
    ":clear_tags fails with non-existant '#{new_resource.user}' user."
  ) unless user_exists?(new_resource.user)
...
action :change_password do
  if user_exists?(new_resource.user)
    cmd =  'rabbitmqctl change_password '
    cmd << "#{new_resource.user} #{new_resource.password}"
...
action :clear_password do
  if user_exists?(new_resource.user)
    cmd = 'rabbitmqctl clear_password #{new_resource.user}'
    execute "rabbitmqctl clear_password #{new_resource.user}" do # ~FC009
      sensitive true
      command cmd
      environment('HOME' => '/var/lib/rabbitmq')
      Chef::Log.debug "rabbitmq_user_clear_password: #{cmd}"
      Chef::Log.info "Clearing RabbitMQ user '#{new_resource.user}' password."
    end
  end
end
