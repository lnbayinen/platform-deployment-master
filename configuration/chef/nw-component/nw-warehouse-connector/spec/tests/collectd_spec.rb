...
describe 'nw-warehouse-connector::collectd' do
  let(:chef_run) do
  ...
  context 'For the python script' do
    file = '/usr/lib/collectd/python/comp_modules/cs_warehouseconnector.py'
    properties = {
      owner: 'root',
      group: 'root',
      mode: 0o444
  ...
  context 'For the collectd configuration fragment' do
    file = '/etc/collectd.d/NwWarehouseConnector.conf'
    properties = {
      owner: 'root',
      group: 'root',
      mode: 0o444
