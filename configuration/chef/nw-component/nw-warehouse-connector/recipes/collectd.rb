...
template '/etc/collectd.d/NwWarehouseConnector.conf' do
  source 'NwWarehouseConnector.conf.erb'
  owner 'root'
  group 'root'
  mode 0o444
  variables(
...
remote_file '/usr/lib/collectd/python/comp_modules/cs_warehouseconnector.py' do
  source 'file:///usr/lib/collectd/python/cs_warehouseconnector.py'
  owner 'root'
  group 'root'
  mode 0o444
  notifies :restart, 'service[collectd]', :delayed
