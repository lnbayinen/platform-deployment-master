...
template '/etc/collectd.d/broker.conf' do
  source 'broker.conf.erb'
  owner 'root'
  group 'root'
  mode 0o444
  variables(
...
remote_file '/usr/lib/collectd/python/comp_modules/cs_broker.py' do
  source 'file:///usr/lib/collectd/python/cs_broker.py'
  owner 'root'
  group 'root'
  mode 0o444
  notifies :restart, 'service[collectd]', :delayed
