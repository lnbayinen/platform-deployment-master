...
#repourl='http://mirror.cogentco.com/pub/linux/centos/6/os/x86_64/repodata/'
repourl=http://vault.centos.org/6.7/os/x86_64/repodata/

# set path of yum.conf
yumConf="${WORKSPACE}/yum.conf.hudson"
# for hashing code base
ngver='ng10.5'
rsacomp=${WORKSPACE}/baseiso/legacy/el6/${CODEVERSION}/el6_upgrade_gold_iso/comps.xml
parsecomp=${WORKSPACE}/baseiso/legacy/el6/${CODEVERSION}/el6_upgrade_gold_iso/parsecomp.py
kscode="${WORKSPACE}/baseiso/legacy/el6/${CODEVERSION}/el6_upgd_ks"
isocode="${WORKSPACE}/baseiso/legacy/el6/${CODEVERSION}/el6_upgrade_gold_iso"
compcode="${WORKSPACE}/baseiso/legacy/el6/${CODEVERSION}/el6_upgrade_gold_iso/comps.xml"
...
newrsacompfile="${mysha256}-c6-x86_64-comps.xml"
if ! [ -s "${workdir}/${newrsacompfile}" ]; then
   cp ${workdir}/comps.xml ${workdir}/${mysha256}-c6-x86_64-comps.xml
   sync
   echo "generated new rsa comps.xml: ${rsacompfile}"
   rsacompfile="${newrsacompfile}"
else
   rsacompfile="${newrsacompfile}"
fi
...
packlist[$count]=$line
let count=$count+1
done < ${WORKSPACE}/${rsacompfile}

let numpacks=${#packlist[@]}

# remove/touch existing packlist file
rm -f ${workdir}/corelist.txt
# kernel package is assumed
echo "writing 'kernel' to corelist.txt" >> ${mylog}
echo "kernel" > ${workdir}/corelist.txt
#echo "fio-2.0.13" >> ${workdir}/corelist.txt
echo "mongodb-org-3.2.7" >> ${workdir}/corelist.txt
echo "mongodb-org-server-3.2.7" >> ${workdir}/corelist.txt
echo "mongodb-org-shell-3.2.7" >> ${workdir}/corelist.txt
echo "mongodb-org-mongos-3.2.7" >> ${workdir}/corelist.txt
echo "mongodb-org-tools-3.2.7" >> ${workdir}/corelist.txt
echo "rsa-audit-plugins" >> ${workdir}/corelist.txt
echo "security-analytics-web-server" >> ${workdir}/corelist.txt
...
do
	...
	# skip optional centos packages
		...
		if [[ `/bin/echo "${packlist[$i]}" | /bin/grep -i -E '^[[:space:]]*<packagereq'` ]] && ! [[ `/bin/echo "${packlist[$i]}" | /bin/grep -i -E '^[[:space:]]*<packagereq[[:space:]]+type="optional">'` ]]; then
			...
			packstr="${packstr#*>}"
			packstr="${packstr%<*}"
			# skip upstream packages not included in downstream disto
			case "$packstr" in
				'yum-rhn-plugin' | 'rhnsd' | 'rhn-setup-gnome' | 'abrt-plugin-mailx' | 'abrt-plugin-sosreport'  | 'openssl-ibmca' | 'iprutils' | 'netxen-firmware' | 'ppc64-diag' | 'ppc64-utils' | 's390utils' | 'yaboot' | 'libjpeg' | 'redhat-access-insights' )
				continue
				;;
				* )
				;;
...
packgroups=( core rsa-sa-archiver rsa-sa-broker rsa-sa-concentrator rsa-sa-core rsa-sa-esa-server rsa-sa-ipmi-tools rsa-sa-log-decoder rsa-sa-lsi-megacli rsa-sa-malware-analysis rsa-sa-malware-analysis-colocated rsa-sa-packet-decoder rsa-sa-remote-logcollector rsa-sa-san-tools rsa-sa-server )
...
do
	unset startline
	startline=`grep -i -n "<id>${group}</id>" ${workdir}/${rsacompfile} | awk '{ print $1 }'`
	startline=${startline//:/}
...
if [[ "$isotype" = 'usb' || "$isotype" = 'dvd' ]]; then
    ...
    #repo-graph -c "${yumConf}" --repoid="${baseRepoID}" > ${workdir}/repodeps.txt
    # parse out jenkins inserted repo definition
    myrepo=`grep '\[.*\]' ${workdir}/yum.conf.hudson | grep -v '\[main\]' | grep -v '\[fbase\]' | awk '{print $1}'`
    myrepo="${myrepo#[}"
    myrepo="${myrepo%]}"
    echo "myrepo = $myrepo"
    repo-graph -c "${yumConf}" --repoid="${myrepo}" > ${workdir}/repodeps.txt
    ...
    yumdownloader -c "${yumConf}" --disablerepo=\* --enablerepo="${myrepo}" `cat "$workdir/corelist.txt"  | tr '\n' ' '`
	...
	#saverstr=${saverstr%\.[a-zA-Z0-9]*}
	#saverstr=${saverstr##*[a-zA-Z]-}
	#saverstr=`echo $saverstr | awk '{print $1}'`
	saverstr=${saverstr#security-analytics-web-server-}
	saverstr=${saverstr%\.el7\.centos\.noarch\.rpm}
	#saverstr=${saverstr}el6
    ...
    newmf="${isodir}/sa-upgrade-${saverstr}${buildnum}-${isotype}.mf"
...
if [[ "$isotype" = 'dvd' || "$isotype" = 'usbboot' ]]; then
   ...
   if ! [[ -e $workdir/binaries/libowb1_0_1-1.1.1.0-1.x86_64.rpm ]]; then
       cd $workdir/binaries
       wget http://cent6build1.netwitness.local/~hudson/buildbin/libowb1_0_1-1.1.1.0-1.x86_64.rpm
       cd $workdir
   fi
    if ! [[ -e $workdir/binaries/rsa-sa-fips-11.0.0.0-3427.1.aee8aeb.el6.x86_64.rpm ]]; then
       cd $workdir/binaries
       wget http://cent6build1.netwitness.local/~hudson/buildbin/rsa-sa-fips-11.0.0.0-3427.1.aee8aeb.el6.x86_64.rpm
       cd $workdir
   fi
   if ! [[ -e $workdir/binaries/MegaCLI && -e $workdir/binaries/menu.c32 && -e $workdir/binaries/isolinux.bin ]] && [[ -e $workdir/binaries/uudecode && -e $workdir/binaries/tr && -e $workdir/binaries/openssl-1.0.0-20.el6_2.5.x86_64.rpm ]] && [[ -e $workdir/binaries/splash.jpg && -e $workdir/binaries/libowb1_0_1-1.1.1.0-1.x86_64.rpm && -e $workdir/binaries/rsa-sa-fips-11.0.0.0-3427.1.aee8aeb.el6.x86_64.rpm ]]; then
...
if [[ "$isotype" = 'dvd' ]]; then
    ...
    cp -f "${kscode}/isolinux.cfg" "$isodir/isolinux/"
    ...
    if ! [[ -s squashfs-root/usr/bin/MegaCLI && -s squashfs-root/usr/bin/uudecode && -s squashfs-root/usr/bin/tr ]] || ! [ -s squashfs-root/rpm/openssl-1.0.0-20.el6_2.5.x86_64.rpm ] || ! [ -s squashfs-root/rpm/libowb1_0_1-1.1.1.0-1.x86_64.rpm ] || ! [ -s squashfs-root/rpm/rsa-sa-fips-11.0.0.0-3427.1.aee8aeb.el6.x86_64.rpm ] || ! [[ -s squashfs-root/usr/bin/optinstall.py && -s $workdir/dvd.hashes.new && `md5sum squashfs-root/usr/bin/optinstall.py | grep "$optinstallmd5"` ]] || ! [[ -s squashfs-root/usr/bin/getosdisk.py && -s $workdir/dvd.hashes.new && `md5sum squashfs-root/usr/bin/getosdisk.py | grep "$getosdiskmd5"` ]]; then
	...
	if ! [ -s squashfs-root/rpm/libowb1_0_1-1.1.1.0-1.x86_64.rpm ]; then
            mkdir -p squashfs-root/rpm
            cp $workdir/binaries/libowb1_0_1-1.1.1.0-1.x86_64.rpm squashfs-root/rpm/
            chmod 644 squashfs-root/rpm/libowb1_0_1-1.1.1.0-1.x86_64.rpm
        fi
	if ! [ -s squashfs-root/rpm/rsa-sa-fips-11.0.0.0-3427.1.aee8aeb.el6.x86_64.rpm ]; then
            mkdir -p squashfs-root/rpm
            cp $workdir/binaries/rsa-sa-fips-11.0.0.0-3427.1.aee8aeb.el6.x86_64.rpm squashfs-root/rpm/
            chmod 644 squashfs-root/rpm/rsa-sa-fips-11.0.0.0-3427.1.aee8aeb.el6.x86_64.rpm
        fi
	if ! [[ -s squashfs-root/usr/bin/optinstall.py && -s $workdir/dvd.hashes.new && `md5sum squashfs-root/usr/bin/optinstall.py | grep "$optinstallmd5"` ]]; then
            ...
            cp ${kscode}/optinstall.py squashfs-root/usr/bin/
	...
	if ! [[ -s squashfs-root/usr/bin/getosdisk.py && -s $workdir/dvd.hashes.new && `md5sum squashfs-root/usr/bin/getosdisk.py | grep "$getosdiskmd5"` ]]; then
            ...
            cp ${kscode}/getosdisk.py squashfs-root/usr/bin/
...
if [[ "$isotype" = 'usbboot' ]]; then
    ...
    cp -f "${kscode}/syslinux.cfg" "$isodir/syslinux"
    ...
    if ! [[ -s squashfs-root/usr/bin/MegaCLI && -s squashfs-root/usr/bin/uudecode && -s squashfs-root/usr/bin/tr ]] || ! [ -s squashfs-root/rpm/openssl-1.0.0-20.el6_2.5.x86_64.rpm ] || ! [ -s squashfs-root/rpm/libowb1_0_1-1.1.1.0-1.x86_64.rpm ] || ! [ -s squashfs-root/rpm/rsa-sa-fips-11.0.0.0-3427.1.aee8aeb.el6.x86_64.rpm ] || ! [[ -s squashfs-root/usr/bin/optinstall.py && -s $workdir/usb.hashes.new && `md5sum squashfs-root/usr/bin/optinstall.py | grep "$optinstallmd5"` ]] || ! [[ -s squashfs-root/usr/bin/getosdisk.py && -s $workdir/dvd.hashes.new && `md5sum squashfs-root/usr/bin/getosdisk.py | grep "$getosdiskmd5"` ]]; then
	if ! [ -s squashfs-root/usr/bin/MegaCLI ]; then
		cp $workdir/binaries/MegaCLI squashfs-root/usr/bin/
        	chmod 755 squashfs-root/usr/bin/MegaCLI
	fi
...
if [[ "$isotype" = 'usbboot' ]]; then
    ...
    if [[ -s sa-upgrade-${saverstr}.${buildnum}.el6-usb.iso ]]; then
        mkisofs -o sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.iso -b syslinux/isolinux.bin -c syslinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -R -J -v -T -joliet-long $isodir
        echo 'getting md5sum of iso ...'
	mymd5sum=`md5sum "sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.iso" | awk '{print $1}'`
	echo '# iso image file manifest' >> "${WORKSPACE}/sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.mf"
	echo "# format: 'file name','md5sum'" >> "${WORKSPACE}/sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.mf"
	echo "sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.iso,$mymd5sum" >> "${WORKSPACE}/sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.mf"
    else
        echo "file sa-upgrade-${saverstr}.${buildnum}.el6-usb.iso not found, canceling build of usbboot"
    fi
else
    if [ -z $url ]; then
        mkisofs -o sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.iso -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -R -J -v -T -joliet-long  $isodir
        implantisomd5 sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.iso
        echo 'getting md5sum of iso ...'
	mymd5sum=`md5sum "sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.iso" | awk '{print $1}'`
	echo '# iso image file manifest' >> "${WORKSPACE}/sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.mf"
	echo "# format: 'file name','md5sum'" >> "${WORKSPACE}/sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.mf"
	...
	if [ ${isotype} = 'dvd' ]; then
		echo "UPSTREAMDVDISO=sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.iso" > ${WORKSPACE}/DVD_ISO.txt
		#scp ${WORKSPACE}/DVD_ISO.txt jenkins@10.4.50.16:/home/jenkins/workspace/02a-of-04-vmwareOVAImageCreation/
		#scp ${WORKSPACE}/DVD_ISO.txt jenkins@10.4.50.16:/home/jenkins/workspace/02b-of-04-openstackImageCreation/
	fi
	for item in "${mflist[@]}"
	do
           echo "$item" >> "${WORKSPACE}/sa-upgrade-${saverstr}.${buildnum}.el6-${isotype}.mf"
	done
    else
         mkisofs -o sa-upgrade-${saverstr}.${buildnum}.el6-url.iso -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -R -J -v -T -joliet-long  $isodir
         implantisomd5 sa-upgrade-${saverstr}.${buildnum}.el6-url.iso
